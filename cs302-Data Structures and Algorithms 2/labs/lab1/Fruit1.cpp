...
int main(int argc, char *argv[])
{
	...
	//cout << f.name << " " << f.quan << " " << f.cost << endl;
	...
	for(int i = 0; i < l.size(); i++){
		...
		totcost = (totcost + (l[i].quan * l[i].cost));
		cout << setw(20) << left << l[i].name << "  " << setw(5) << left << setprecision(2) << fixed << l[i].quan << "x " << setw(4) << left << setprecision(2) << fixed << l[i].cost << " = " << setw(7) << right << setprecision(2) << fixed << (l[i].quan * l[i].cost) << "  : " << setw(8) << right << setprecision(2) << fixed << totcost << endl;
	}
